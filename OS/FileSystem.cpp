...
useropen openfilelist[MAXOPENFILE];  // 用户已经打开文件表数组
...
void my_cd(char *dirname)//更改当前目录，参数是新的当前目录的目录名。
{
    char *dir;
    int fd;  //文件描述符
    ...
    {
        while(curdir)
            curdir = my_close(curdir);//循环关闭root下的打开文件； 
...
void my_mkdir(char *dirname)//创建子目录，参数是新建目录的目录名。
{
    fcb *fcbptr;//文件控制块
    fat *fat1, *fat2;//文件分配表
    time_t now;//系统时间结构体
    struct tm *nowtime;//系统时间结构体
    char text[MAXTEXT];//存放打开表项中的内容。 
    unsigned short blkno;//盘块
    int rbn, fd, i;
	//文件分配表起始位置; 
    fat1 = (fat *)(myvhard + BLOCKSIZE);
    fat2 = (fat *)(myvhard + 3 * BLOCKSIZE);
    openfilelist[curdir].count = 0;//读写指针置 0; 读写指针在文件中的位置？？？
    rbn = do_read(curdir, openfilelist[curdir].length, text);//读入当前目录的用户打开表内容到内存(存到 text里); 实际读出的字节数。
    fcbptr = (fcb *)text;//指向fcb
    ...
    fd = my_open(dirname);//返回新建立的目录文件在用户打开文件数组的下标......//取得该目录的用户打开表项序号; 
    if(fd == -1)
        return;
    fcbptr = (fcb *)malloc(sizeof(fcb));//建立新目录的'.','..'目录.....//在新建的目录文件所分配到的磁盘块中建立目录项"."和 ".." ; 
	//对新建目录.进行fcb信息的初始化。
    now = time(NULL);
    nowtime = localtime(&now);
...
void my_rmdir(char *dirname)//删除子目录，要删除的目录名。
{
    fcb *fcbptr,*fcbptr2;//文件控制块
    fat *fat1, *fat2, *fatptr1, *fatptr2;//文件分配表
    char text[MAXTEXT], text2[MAXTEXT];//打开表项中的内容 
    unsigned short blkno;//盘块
    int rbn, rbn2, fd, i, j;//当前目录文件的字节数，//文件的描述符
    fat1 = (fat *)(myvhard + BLOCKSIZE);
    fat2 = (fat *)(myvhard + 3 * BLOCKSIZE);
    ...
    if(i == rbn / sizeof(fcb))//没有找到要删除的目录
    {
        printf("Error,the directory is not exist.\n");
        return;
    }
    fd = my_open(dirname);//目录在当前打开文件数组中的下标。//打开要删的目录； 
...
{
    fcb *fcbptr;//文件控制块
    char text[MAXTEXT];//打开表项的内容
    int rbn, i;
    openfilelist[curdir].count = 0;
    rbn = do_read(curdir, openfilelist[curdir].length, text);//读取当前目录的所有内容
    fcbptr = (fcb *)text;//当前目录的手指针
    for(i = 0; i < rbn / sizeof(fcb); i++)
    {
        if(fcbptr->free)//如果目录项已分配（非空）; 
        {
            if(fcbptr->attribute & 0x20)//目录文件
